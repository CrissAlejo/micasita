...
function Ingresos(props) {
  ...
  const handleChangeTab = (event, value) => {
    value -= 1;
    if(value<0){
      FirestoreService.getAllIngresos(settings.idConjunto,
        {
          next: (querySnapshot) => {
  ...
  const confir = () => {
    ...
    if (elm) {
      FirestoreService.deleteIngresoByConjunto(settings.idConjunto, elm).then(
        (docRef) => {
          setLoading(false);
        }
      );
      ...
      <Grid container spacing="3">
        ...
        <AppBar position="static" color="default">
          <Tabs
            ...
            textColor="primary"
          >
            <Tab label={'Todos los ingresos'} icon={<HomeIcon />} />
            {cuentas
